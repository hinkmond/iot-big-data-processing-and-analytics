...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: kafka-direct-iotmsg-hum-sensor.py <broker_list> <topic>", file=sys.stderr)
        exit(-1)
    ...
    dataValues = jsonLines.filter(lambda x: re.findall(r"Humidity.*", x, 0)) 
    dataValues.pprint(num=13824)

    # Parse out just the value without the JSON key
    parsedValues = dataValues.map(lambda x: re.sub(r"\"Humidity\":", "", x))

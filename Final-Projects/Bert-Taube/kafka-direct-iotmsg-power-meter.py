...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: kafka-direct-iotmsg-power-meter.py <broker_list> <topic>", file=sys.stderr)
        exit(-1)
    ...
    dataValues = jsonLines.filter(lambda x: re.findall(r"Power.*", x, 0)) 
    dataValues.pprint(num=12432)

    # Parse out just the value without the JSON key
    parsedValues = dataValues.map(lambda x: re.sub(r"\"Power\":", "", x))
